...
module.exports = {
  ...
  create: function (context) {
    ...
    const sortNodeArgumentValue = (node, arg = null) => {
      ...
      let prefix = '';
      let suffix = '';
      let isSortingArray = false;
      const isSimpleClassNameLiteral = (el) => {
        return typeof el === 'object' && el.type === 'Literal' && el.value.split(' ').length === 1;
      }
      ...
      if (arg === null) {
        ...
        switch (arg.type) {
          ...
          case 'ArrayExpression':
            const notSimpleClassNameLiteral = arg.elements.filter((el) => !isSimpleClassNameLiteral(el));

            // Any array element that is not just a string with single classname is recursively sorted.
            notSimpleClassNameLiteral.forEach((el) => {
              sortNodeArgumentValue(node, el);
            });

            const simpleClassNameLiterals = arg.elements.filter((el) => isSimpleClassNameLiteral(el));

            if (simpleClassNameLiterals.length > 0) {
              originalClassNamesValue = simpleClassNameLiterals.map((el) => {
                return el.value;
              }).join(' ');
      ...
      if (originalClassNamesValue !== validatedClassNamesValue) {
        ...
        context.report({
          ...
          fix: function (fixer) {
            if (isSortingArray) {
              const sortedClassNames = validatedClassNamesValue.split(' ');
              const fixers = [];

              arg.elements.filter((el) => isSimpleClassNameLiteral(el)).forEach((el, i) => {
                if (el.value !== sortedClassNames[i]) {
                  fixers.push(fixer.replaceTextRange([el.range[0] + 1, el.range[1] - 1], sortedClassNames[i]));
                }
              });
